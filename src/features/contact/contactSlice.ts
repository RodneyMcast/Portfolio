...
export const submitContactForm = createAsyncThunk<
  ...
  ContactFields,
  { rejectValue: string }
>("contact/submitContactForm", async (_, { rejectWithValue }) => {
  try {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(submitContactForm.rejected, (state, action) => {
        state.status = "failed";
        state.errorMessage =
          action.payload ?? "Unable to send the message right now.";
      });
  },
});

export const { setField, validateForm, resetForm, clearStatus } =
